...
public class LoginPageTest extends TestBase {
	...
	public void verifyLoginPageComponents() {
		...
		assertTrue(loginPage.visiblityOfLoginPageComponents(), "Fail to load login components");

	}
	
	
	//(priority = 2, description = "Verify valid login functionality", groups = { "Functional" })
	public void verifyValidLogin() throws Exception {
		...
		assertTrue(loginPage.verifyLogin(username, password), "Login functionality failed with valid credentials");
	...
	//@Test(priority = 3, description = "Verify valid login functionality after Enter key Press", groups = { "Functional" })
	public void verifyValidLoginEnterKeyPress() throws Exception {
		...
		assertTrue(loginPage.verifyLoginWithEnterKeyPress(username, password), "Login functionality failed with valid credentials");
	...
	//@Test(priority = 4, description = "Verify Browser back functionality after Login", groups = { "Functional" })
	public void clickBrowserBackTest() throws Exception {
		...
		assertTrue(loginPage.clickBrowserBackTest(username, password), "Login functionality failed with valid credentials");
	...
	public void InvalidUserValidPassword() throws Exception {
		...
		String username = "abh.ranade" ;// EnvironmentConfig.environmentSetup().get("INUSERNAME");
		String password = "YELLO#12" ;//EnvironmentConfig.environmentSetup().get("PASSWORD");
		...
		//new WebDriverWait(driver, Duration.ofSeconds(50)).until(ExpectedConditions.visibilityOf(loginPage.msgInvalidCredentials));
	...
	public void validUserInvalidPassword() throws Exception {
		...
		String username = "abhayranade" ;// EnvironmentConfig.environmentSetup().get("INUSERNAME");
		String password = "WHITE#12" ;//EnvironmentConfig.environmentSetup().get("PASSWORD");
	...
	public void InvalidUserInvalidPassword() throws Exception {
		...
		String username = "abh.ranade1" ;// EnvironmentConfig.environmentSetup().get("INUSERNAME");
		String password = "WHITE#12" ;//EnvironmentConfig.environmentSetup().get("PASSWORD");
	...
	public void showPasswordFunctionality() throws Exception {
		...
		String username = "" ;// EnvironmentConfig.environmentSetup().get("INUSERNAME");
		String password = "WHITE#12" ;//EnvironmentConfig.environmentSetup().get("PASSWORD");
		...
		loginPage.showEyeIconPassword.click();
		Thread.sleep(3000);

		//assertTrue(loginPage.txtPassword.ge));
	...
	public void VerifyChooseDifferentPortal() throws Exception {
		...
		Thread.sleep(3000);
		String expectedURL = "https://sandbox.ciranet.com/v2/portalselection";
	...
	public void VerifyForgotPasswordButtonClick() throws  Exception {
		...
		Thread.sleep(2000);
		String expectedURL = "https://sandbox.ciranet.com/v2/managementportal/user/forgotpassword";
		...
		Thread.sleep(1000);
		loginPage.btnBackToLogin.click();
		Thread.sleep(2000);
	...
	public void VerifyForgotPasswordPageComponents() throws Exception {
		...
		assertTrue(loginPage.visiblityOfForgotPasswordPageComponents(), "Fail to load Forgot Password Page components");
		loginPage.btnBackToLogin.click();
		Thread.sleep(2000);
	}
	
	@SuppressWarnings("deprecation")
	//@Test(priority = 13, description = "Simultaneous Login On Same Browser", groups = { "Smoke" })
	public void verifySimultaneousLoginOnSameBrowser() throws Exception 
	{
	...
	@SuppressWarnings("deprecation")
	//@Test(priority = 14, description = "Verify user is able to login on different browsers with different credentials", groups = { "Functional" })
	public void verifyLoginDiffUsersOnDifferentBrowsers() throws Exception {
		...
		assertTrue(loginPage.verifyLoginDiffUsersOnDifferentBrowsers(username, password), "Login functionality failed with different credentials different browser");
		driver.manage().timeouts().pageLoadTimeout(20, TimeUnit.SECONDS);
